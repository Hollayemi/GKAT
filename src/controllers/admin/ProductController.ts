/**
 * ProductController – every mutating handler logs to ActivityLog.
 * Read-only handlers (getProduct, getProducts, …) are intentionally not logged
 * to avoid log noise; add ACTIONS.PRODUCT_VIEWED calls if your audit policy needs it.
 */
...
import {
  getProductPipeline, getStockHistoryPipeline,
  lastMonthSalesPipeline, previousMonthSalesPipeline,
  salesReveniuePipeline, salesTrendPipeline, stockHistoryPipeline,
} from './pipeline';
...
async function findNearestRegionId(
  ...
  for (const region of regions) {
    ...
    if (!nearest || distanceKm < nearest.distanceKm)
      nearest = { regionId: region._id as mongoose.Types.ObjectId, regionName: region.name, distanceKm };
  }
  return nearest;
}
...
async function generateUniqueSku(prefix = 'GK'): Promise<string> {
  ...
  while (!isUnique) {
    ...
    sku = `${prefix}-${rand}`;
    if (!(await Product.findOne({ sku }))) isUnique = true;
  }
  return sku;
}
...
async function buildProductOverview() {
  ...
  const [
    totalProducts, inStockCount, lowStockCount, outOfStockCount, categoryCount,
    lastMonthTotal, lastMonthInStock, lastMonthLowStock, lastMonthOutOfStock,
  ] = await Promise.all([
  ...
  return {
    totalProducts:  { value: totalProducts,  change: pctChange(totalProducts,  lastMonthTotal),      color: 'purple' },
    inStock:        { value: inStockCount,   change: pctChange(inStockCount,   lastMonthInStock),    color: 'orange' },
    lowStock:       { value: lowStockCount,  change: pctChange(lowStockCount,  lastMonthLowStock),   color: 'cyan'   },
    outOfStock:     { value: outOfStockCount,change: pctChange(outOfStockCount,lastMonthOutOfStock), color: 'red'    },
    categories:     { value: categoryCount,  change: '- vs last month',                              color: 'blue'   },
  };
}

const CSV_TEMPLATE_HEADERS = [
  'productName','sku','productId','brand','category','description',
  'salesPrice','unitType','unitQuantity','stockQuantity','minimumStockAlert','tags','status',
];
...
// ─── READ handlers (no logging by design) ─────────────────────────────────────

export const getProducts = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  const {
    category, status, search, minPrice, maxPrice, tags,
    page = 1, limit = 20, sort = '-createdAt',
    lat, lng, regionId, includeOverview = 'false',
  } = req.query;
  ...
  else if (status === 'low-stock')    { query.stockQuantity = { $gt: 0, $lte: 10 }; delete query.status; }
  else if (status === 'out-of-stock') { query.stockQuantity = 0; delete query.status; }
  ...
  if (tags) query.tags = { $in: (tags as string).split(',').map((t) => t.trim()) };
  ...
  if (regionId) {
    ...
    const userLat = parseFloat(lat as string), userLng = parseFloat(lng as string);
    ...
    if (nearest) { resolvedRegionId = nearest.regionId; resolvedRegionName = nearest.regionName; distanceKm = nearest.distanceKm; }
  }

  if (resolvedRegionId)
    query['regionalDistribution'] = { $elemMatch: { region: resolvedRegionId, mainProduct: { $gt: 0 } } };
  ...
  const [products, total] = await Promise.all([
    Product.aggregate(getProductPipeline({ query, skip, sort: { [sort as string]: 1 }, limitNum })),
  ...
  if (includeOverview === 'true') responseData.overview = await buildProductOverview();
...
export const getProductsOverview = asyncHandler(async (_req: Request, res: Response) => {
  (res as AppResponse).data(await buildProductOverview(), 'Products overview retrieved successfully');
});
...
export const getProduct = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  const product = await Product.findById(req.params.id)
    ...
    .populate('regionalDistribution.region', 'name coordinate').lean();
...
export const getProductBySku = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  const product = await Product.findOne({ sku: req.params.sku.toUpperCase() }).populate('createdBy', 'name email');
  if (!product) return next(new AppError('Product not found', 404));
  (res as AppResponse).data({ product }, 'Product retrieved successfully');
});
...
export const createProduct = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  ...
  await logActivity(req, {
    ...
    description: `Created product "${product.productName}" (SKU: ${product.sku})`,
    targetId:    product._id.toString(),
    targetType:  'Product',
    targetName:  product.productName,
    after: {
      productName: product.productName, sku: product.sku,
      salesPrice: product.salesPrice, stockQuantity: product.stockQuantity,
      status: product.status, category: product.category?.toString(),
    },
  });
...
export const updateProduct = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (req.body.sku && req.body.sku.toUpperCase() !== product.sku) {
    ...
    if (existing) return next(new AppError('Product with this SKU already exists', 409));
  }

  if (req.files && Array.isArray(req.files) && req.files.length > 0) {
    try {
      if (product.images?.length) await CloudinaryService.deleteMultipleImages(product.images);
...
export const deleteProductFunction = async (id: string): Promise<any> => {
  ...
  if (product.images?.length) {
    try { await CloudinaryService.deleteMultipleImages(product.images); } catch (err) { console.error(err); }
  }
  await product.deleteOne();
  return product; // Return the doc so callers can log the name/sku
};

export const deleteProduct = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  // Fetch first so we have the name/sku for the log
  const product = await Product.findById(req.params.id).lean();
  if (!product) return next(new AppError('Product not found', 404));

  await deleteProductFunction(req.params.id);

  await logActivity(req, {
    action:      ACTIONS.PRODUCT_DELETED,
    description: `Deleted product "${product.productName}" (SKU: ${product.sku})`,
...
export const updateStock = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (variantId && variantStock !== undefined && product.variants) {
    ...
    variantName = `${variant.unitQuantity} ${variant.unitType}`;
  ...
  await logActivity(req, {
    action: ACTIONS.PRODUCT_STOCK_UPDATED,
    description: variantId
      ? `Updated stock for variant "${variantName}" of "${product.productName}" to ${variantStock}`
      : `Updated stock for "${product.productName}" from ${previousStock} → ${stockQuantity}`,
    targetId:   product._id.toString(),
    targetType: 'Product',
    targetName: product.productName,
    before:     { stockQuantity: previousStock },
    after:      { stockQuantity: product.stockQuantity },
    metadata:   variantId ? { variantId, variantStock } : undefined,
  });
...
export const addVariant = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  ...
  await logActivity(req, {
    action:      ACTIONS.PRODUCT_VARIANT_ADDED,
    description: `Added variant (SKU: ${req.body.sku}) to product "${product.productName}"`,
    targetId:    product._id.toString(),
    targetType:  'Product',
    targetName:  product.productName,
    metadata:    { variantSku: req.body.sku, variantUnitType: req.body.unitType },
  });
...
export const updateVariant = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (typeof variantIndex === 'undefined' || variantIndex === -1)
    return next(new AppError('Variant not found', 404));

  const variant = product.variants?.[variantIndex];
  if (!variant) return next(new AppError('Variant not found', 404));

  const beforeVariant = (variant as { toObject?: () => object }).toObject?.() ?? { ...variant };
  Object.assign(variant, req.body);
  await product.save();

  await logActivity(req, {
    action:      ACTIONS.PRODUCT_VARIANT_UPDATED,
    description: `Updated variant on product "${product.productName}"`,
    targetId:    product._id.toString(),
    targetType:  'Product',
    targetName:  product.productName,
...
export const deleteVariant = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  ...
  await logActivity(req, {
    action:      ACTIONS.PRODUCT_VARIANT_DELETED,
    description: `Deleted variant from product "${product.productName}"`,
    targetId:    product._id.toString(),
    targetType:  'Product',
    targetName:  product.productName,
    before:      { variantId: req.params.variantId, ...(deletedVariant ?? {}) },
  });
...
export const updateRegionalDistribution = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  ...
  const before = product.regionalDistribution;
  product.regionalDistribution = req.body.regionalDistribution;
  await product.save();

  await logActivity(req, {
    action:      ACTIONS.PRODUCT_UPDATED,
    description: `Updated regional distribution for product "${product.productName}"`,
    targetId:    product._id.toString(),
    targetType:  'Product',
    targetName:  product.productName,
    before:      { regionalDistribution: before },
    after:       { regionalDistribution: req.body.regionalDistribution },
  });

  (res as AppResponse).data({ product }, 'Regional distribution updated successfully');
});

// ─── LOW STOCK / BULK UPDATE / DEALS ─────────────────────────────────────────

export const getLowStockProducts = asyncHandler(async (req: Request, res: Response) => {
  const products = await Product.find({ $expr: { $lte: ['$stockQuantity', '$minimumStockAlert'] } })
    .populate('createdBy', 'name email');
  (res as AppResponse).data({ products, count: products.length }, 'Low stock products retrieved successfully');
});
...
export const bulkUpdateProducts = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  ...
  await logActivity(req, {
    action:      ACTIONS.PRODUCT_BULK_UPDATED,
    description: `Bulk-updated ${result.modifiedCount} product(s). Fields changed: ${Object.keys(updates).join(', ')}`,
    metadata:    { productIds, updates, modifiedCount: result.modifiedCount },
  });
...
export const setDealsOfTheDay = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  ...
  await logActivity(req, {
    action:      ACTIONS.PRODUCT_DEAL_SET,
    description: `Set deal of the day for "${product?.productName ?? productId}" at ${percentage}% off`,
    targetId:    productId,
    targetType:  'Product',
    targetName:  product?.productName,
    metadata:    { percentage, startDate, endDate, status },
  });

  const dealsProducts = await Product.find({
    dealInfo: { $exists: true }, 'dealInfo.status': 'active', status: 'active',
  }).populate('category');
...
export const getDealsOfTheDay = asyncHandler(async (req: Request, res: Response) => {
  ...
  const deals = await Product.find({ 'dealInfo.status': 'active', status: 'active', stockQuantity: { $gt: 0 } })
    .sort({ dealsSetAt: -1 }).limit(parseInt(limit as string));
  (res as AppResponse).data({ deals, count: deals.length }, 'Deals of the day retrieved successfully');
});
...
export const removeFromDeals = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  ...
  await logActivity(req, {
    action:      ACTIONS.PRODUCT_DEAL_REMOVED,
    description: `Removed deal of the day from product "${product.productName}"`,
    targetId:    product._id.toString(),
    targetType:  'Product',
    targetName:  product.productName,
  });
...
// ─── PRODUCT PREVIEW / STOCK HISTORY (read-only) ─────────────────────────────
...
export const getProductPreview = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (previousRevenue > 0) percentageChange = ((currentMonthRevenue - previousRevenue) / previousRevenue) * 100;
  else if (currentMonthRevenue > 0) percentageChange = 100;
  ...
  let stockStatus = product.stockQuantity === 0 ? 'out-of-stock'
    : product.stockQuantity <= product.minimumStockAlert ? 'low-stock' : 'in-stock';

  const regionalInventory = product.regionalDistribution?.map((region) => ({
    region: region.region, currentStock: region.mainProduct, lastRestocked: null, manager: null,
  ...
  (res as AppResponse).data(
    {
      product: {
        _id: product._id, productId: product.productId, sku: product.sku,
        productName: product.productName, brand: product.brand, category: product.category,
        description: product.description, images: product.images, status: product.status,
        tags: product.tags, salesPrice: product.salesPrice, unitType: product.unitType,
        unitQuantity: product.unitQuantity, stockQuantity: product.stockQuantity, totalStock,
        minimumStockAlert: product.minimumStockAlert, stockStatus, variants: product.variants,
        createdBy: product.createdBy, updatedBy: product.updatedBy,
      ...
      analytics: {
        ...
        salesTrend: salesTrend.map((t) => ({ month: t.month, revenue: t.revenue, quantity: t.quantity })),
      },
      ...
      overview: {
        ...
        availabilityStatus: product.status, tags: product.tags,
      },
    },
...
export const getStockHistory = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  ...
  const daysAgo = new Date(); daysAgo.setDate(daysAgo.getDate() - Number(days));
...
export const downloadImportTemplate = asyncHandler(async (_req: Request, res: Response) => {
  const exampleRow: Record<string, string> = {
    productName: 'Indomie Instant Noodles', sku: '', productId: '',
    brand: 'De-United Foods', category: 'Food & Beverages',
    description: 'A pack of 40 indomie chicken flavour noodles.',
    salesPrice: '3500', unitType: 'pack', unitQuantity: '40',
    stockQuantity: '200', minimumStockAlert: '20',
    tags: 'noodles,instant,food', status: 'active',
  };
  const csv = stringify([exampleRow], { header: true, columns: CSV_TEMPLATE_HEADERS });
...
export const importProductsFromCsv = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  ...
  const required = ['productName', 'salesPrice', 'unitType', 'unitQuantity', 'stockQuantity', 'description'];
  const missing  = required.filter((h) => !Object.keys(rows[0]).includes(h));
  if (missing.length) return next(new AppError(`Missing required CSV columns: ${missing.join(', ')}`, 400));
    ...
    try {
      ...
      if (!sku) {
        ...
        if (await Product.findOne({ sku })) {
          errors.push({ row: rowNum, message: `SKU "${sku}" already exists — skipped` }); continue;
        }
      }
      ...
      if (categoryName) {
        if (categoryCache.has(categoryName)) {
          ...
          if (!cat) { errors.push({ row: rowNum, message: `Category "${categoryName}" not found — row skipped` }); continue; }
      ...
      if (!productName) { errors.push({ row: rowNum, message: 'productName is required' }); continue; }

      const salesPrice = parseFloat(row.salesPrice);
      if (isNaN(salesPrice) || salesPrice < 0) { errors.push({ row: rowNum, message: 'salesPrice must be a valid positive number' }); continue; }

      const stockQuantity = parseInt(row.stockQuantity);
      if (isNaN(stockQuantity) || stockQuantity < 0) { errors.push({ row: rowNum, message: 'stockQuantity must be a valid non-negative integer' }); continue; }

      const validUnitTypes = ['single', 'pack', 'carton', 'kg', 'litre', 'box'];
      const unitType = row.unitType?.trim().toLowerCase();
      if (!validUnitTypes.includes(unitType)) { errors.push({ row: rowNum, message: `unitType must be one of: ${validUnitTypes.join(', ')}` }); continue; }

      const validStatuses = ['active', 'inactive', 'draft'];
      const status = row.status?.trim().toLowerCase() || 'draft';
      if (!validStatuses.includes(status)) { errors.push({ row: rowNum, message: `status must be one of: ${validStatuses.join(', ')}` }); continue; }

      const productDoc: any = {
        productName, sku, brand: row.brand?.trim() || undefined, category: categoryId,
        description: row.description?.trim() || `${productName} — imported from CSV`,
        salesPrice, unitType, unitQuantity: parseFloat(row.unitQuantity) || 1,
        stockQuantity, minimumStockAlert: parseInt(row.minimumStockAlert) || 20,
        tags: row.tags ? row.tags.split(',').map((t: string) => t.trim().toLowerCase()).filter(Boolean) : [],
        status, images: [], createdBy: req.user.id, variants: [], regionalDistribution: [],
      };

      if (row.productId?.trim()) {
        if (await Product.findOne({ productId: row.productId.trim() })) {
          errors.push({ row: rowNum, message: `productId "${row.productId.trim()}" already exists — skipped` }); continue;
        }
  ...
  await logActivity(req, {
    action:      ACTIONS.PRODUCTS_IMPORTED,
    description: `Imported ${created.length} product(s) via CSV. ${errors.length} row(s) failed.`,
    metadata: {
      importedCount:  created.length,
      failedCount:    errors.length,
      totalRows:      rows.length,
      createdIds:     created.map((p) => p._id),
    },
  });

  (res as AppResponse).data(
    { imported: created.length, failed: errors.length, created, errors },
...
export const exportProductsToCsv = asyncHandler(async (req: Request, res: Response) => {
  const { status, category, search, minPrice, maxPrice } = req.query;
  ...
  const csv      = stringify(rows, { header: true, columns: CSV_TEMPLATE_HEADERS });

  await logActivity(req, {
    action:      ACTIONS.PRODUCTS_EXPORTED,
    description: `Exported ${products.length} product(s) to CSV`,
    metadata:    { filters: { status, category, search, minPrice, maxPrice }, exportedCount: products.length },
  });
